...
Note that we have both mock object testing in `test_web.py` and fuzz testing in the `src/Tests.py`, so advacned testing approaches are used. Anything with `str(secrets.token_hex(16))` is just purely random, so fuzz testing. `MagicMock()` is used for mock objects.
